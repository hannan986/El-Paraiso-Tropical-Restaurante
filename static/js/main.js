...
function renderMenu(category) {
  ...
  items.forEach((item, i) => {
    ...
    const imgInner = item.imageFile
      ? `<img src="images/${encodeURIComponent(item.imageFile)}" alt="${item.name}" loading="lazy" onerror="menuImgError(this,'${item.bg}','${item.emoji}')" />`
...
/* =============================================
   IMAGE FALLBACK — if image fails, show emoji
   ============================================= */
function menuImgError(img, bg, emoji) {
...
window.menuImgError = menuImgError;
